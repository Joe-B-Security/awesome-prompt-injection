...
- [Guard Bands](https://github.com/Cryptix-Security/guard-bands) - Cryptographic boundaries separating untrusted content from trusted instructions: untrusted input is wrapped in HMAC-SHA256-signed markers and verified server-side before it can reach a tool call ("prepared statements for LLMs"), rather than prompting a model to classify inputs. The MAC binds content plus all marker metadata (version, key id, issuer, issued/expiry), so markers can't be forged, tampered with, or downgraded; tamper, replay, and expiry fail closed. POC with a wrap/verify/chat API, SSO, audit logging, and pluggable secrets. MIT.
