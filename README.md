...
- [Agent-Directed Manipulation](https://github.com/perpensum/agent-directed-manipulation) - Open definition of the *content* side of indirect prompt injection: what counts as text placed on a web page to steer a machine reader, as distinct from legitimate machine-readable self-presentation (structured data, `llms.txt`, ordinary marketing copy). Classification uses two mechanically decidable axes — region (first-party/third-party) and visibility (visible/hidden) — and no subjective axis, so independent implementations reach the same verdict on the same page. Ships 18 conformance cases, **13 of which expect no finding at all**, plus a dependency-free reference implementation (`curl -s <url> | node scan.mjs`). Known limits are published rather than hidden behind a claim of detection. CC BY 4.0 / MIT.
