...
- [agent-leash](https://github.com/tonydzi/agent-leash) - LEASH-8, an 8-domain control model for agents with delegated authority, centred on plan-vs-authorize: the model *proposes* actions but an independent deterministic gate *authorizes* them, so a poisoned tool description cannot self-authorize. Out-of-band approval tokens bound to the request/session and egress leak-scanning contain an indirect injection even when the model obeys it. Includes a scorecard and approval-design checklist; patterns sanitized from a live production agent operation. MIT. Reproducible attack-and-containment demo: [leash-poc](https://github.com/tonydzi/leash-poc).
